...
class TelegramService {
     ...
     * 재시도 로직을 포함한 메시지 전송
     */
    async sendMessageWithRetry(message, imageUrl, tweetId, accountName, retryCount = 0) {
        const maxRetries = 3;
        ...
        try {
            ...
            if (errorMessage.includes('429') && retryCount < maxRetries) {
                ...
                logger_1.logger.warn('DEBUG: 텔레그램 API 제한으로 재시도 대기', {
                    tweetId: tweetId,
                    accountName: accountName,
    ...
    formatTweetMessage(tweet, accountName, showBothLanguages = false, originalEnglishContent) {
        ...
        // HTML 정리 전후 길이 로깅
        logger_1.logger.info('DEBUG: HTML 정리 전후 길이', {
